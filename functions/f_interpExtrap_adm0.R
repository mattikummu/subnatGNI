...
f_interpExtrap_adm0 <- function(nameIndic, yearRange = 1990:2021) {
  ...
  # interpolate
  indicInterp <-   f_interpAdm0(HDIadm0Comb = adm0_comb_filled, 
  ...
  dataLess5 <- indicInterp %>%      filter(year %in% yearRange) %>% 
    select(iso3, year, !!nameIndic) %>% 
    rename(value := !!nameIndic) %>% 
    group_by(iso3) %>% 
    summarise(across(where(is.numeric), ~sum(!is.na(.)))) %>% 
  ...
  dataNearlyFull <- indicWider %>% 
    ...
    #mutate(nearFullTimeser = ifelse(!is.na(n1998)&!is.na(n2018), 1, 0 )) %>% 
    mutate(nearFullTimeser = ifelse(!is.na(.[[4]])&!is.na(.[[ncol(indicWider)-3]]), 1, 0 )) %>% 
  ...
  dataFull_NearlyFull_Rest_Less5 <- dataFull_NearlyFull %>% 
    ...
    set_names('iso3', paste0(yearRange)) %>% 
